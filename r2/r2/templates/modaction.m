...
<%namespace file="utils.m" import="timestamp, plain_link"/>
...
<tr class="modactions" style="background-color: ${thing.bgcolor}" data-fullname="${thing.fullname}">
  ...
  %if thing.is_multi:
    <td class="subreddit">${plain_link('/' + g.brander_community_abbr + '/' + thing.subreddit.name, thing.subreddit.path + 'about/log', title=thing.subreddit.name)}</td></td>
  %endif
