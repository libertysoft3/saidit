  ...
  <div class="score">
    ${unsafe(Score.PERSON_LABEL % dict(num = format_number(thing.a.score),
                                       persons = websafe(ungettext("point", "points",
                                                                   thing.a.score))))}
    &#32;(${_("%(percent)s " + g.voting_upvote_name) % dict(percent=format_percent(thing.a.upvote_ratio))} | ${_("%(percent)s " + g.voting_controversial_name) % dict(percent=format_percent(1-thing.a.upvote_ratio))})
  </div>
